...
cell_type = pd.read_csv("data/scRNAseq/scRNAseq_cell_cluster_annotation.csv", index_col=0)
...
plt.savefig('outputs_stan/mRNA_ranking_across_cell_cluster_scRNAseq_annotation_plot.png')
plt.close()
...
color_keys = [ 'BNC2+ZFPM2+ fibroblast', 'CD56 NK cell', 'Capillary EC', 'OCE S100A9+MUC16+ epithelial cell',
               'OCE cycling epithelial cell', 'Plasma cell', 'cDC2']
...
plt.savefig('outputs_stan/annotated_7_cell_type_spatial_plot.png')
plt.close()

adata.write("outputs_stan/ovarian_cancer_annotated_7_cell_type_cluster_data.h5ad")

#write out cell type proportions for spots
obs_subset = adata.obs[color_keys]
obs_subset.to_csv('data/scRNAseq/annotated_cell_type.csv')

print(pd.read_csv('data/scRNAseq/annotated_cell_type.csv', index_col=0).head())
#(4674, 7)